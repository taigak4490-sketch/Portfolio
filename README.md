...
2. DynamoDB × S3 による「疎結合」なアーキテクチャ
...
4. セキュリティ対策
    セキュリティ対策として tfsec を導入しています。HIGH の脆弱性はワイルドカード関連に対するエラーであったため、拡張性（動的なファイルパスへの対応）を優先し、ポリシーの適用範囲を限定した上で ignore 設定を適用しました。 
    MEDIUM 以下はログ関連であったため、コストが高額になるのを避けるために意図的に残しています。
